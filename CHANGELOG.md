...
### 新增
- 內建第三方技能 **`humanizer-zh-tw`**（MIT，來源 kevintsai1202/Humanizer-zh-TW）：去除中文輸出的 AI 痕跡、潤飾為自然繁體中文。
- `taiwan-finance-context` 新增 **§八 台灣用詞規範**（避免簡體／中國用語與殘留英文的對照表）與 **§九 輸出潤飾**（呼叫 humanizer 但須保留數字、中立、免責）。

### 說明
- 潤飾規則明確要求：財經內容潤飾時不可刪改數字、不可加入主觀投資意見、必保留免責與 `[需查證]`。
