...
3. **Alternative search methods if IR site is unclear:**
   ...
   - 公開資訊觀測站（MOPS）: Search for company and look at most recent 季報 or 年報 filing date
...
- Look for most recent 季報 (quarterly) or 年報 (annual if Q4)
...
- **季報 or 年報 filing** - On 公開資訊觀測站（MOPS） (sec.gov/edgar/searchedgar/companysearch.html)
  - Search by ticker symbol
  - For quarters 1-3: Look for most recent 季報
...
- **Consensus estimates** - From TWSE／FinMind, TWSE MCP（twstock）, Refinitiv, or Yahoo Finance
...
- [ ] ✅ **季報/年報 filing date**: _______________ (MUST be same quarter as release)
...
EPS (T-IFRS)          $X.XX       $X.XX      $X.XX        $X.XX
...
- [ ] All MOPS 公開資訊申報 hyperlinked to MOPS viewer
