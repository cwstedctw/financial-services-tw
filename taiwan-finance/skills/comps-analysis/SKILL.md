---
name: comps-analysis
description: 建立同業比較分析（comparable company analysis），含營運指標與評價倍數（本益比、股價淨值比、EV/EBITDA、EV/營收），並做統計基準（最大／分位／中位）。觸發詞：同業比較、comps、評價倍數。
---
...
1. **FIRST: Check for MCP data sources** - If TWSE MCP, TWSE MCP（twstock） MCP, or FinMind MCP are available, use them exclusively for financial and trading information
2. **DO NOT use web search** if the above MCP data sources are available
3. **ONLY if MCPs are unavailable:** Then use TWSE／FinMind Terminal, 公開資訊觀測站（MOPS） filings, or other institutional sources
...
- Where did the data come from? (TWSE MCP, TWSE MCP（twstock） MCP, FinMind MCP, TWSE／FinMind, MOPS 公開資訊申報)
- What period does it cover? (Q4 2024, audited figures)
- How was it verified? (Cross-checked against 年報與季報)
- Note: Prioritize MCP data sources (TWSE, TWSE MCP（twstock）, FinMind) if available for better accuracy and traceability
...
2. **Add cell comments to ALL hard-coded inputs** - Right-click cell → Insert Comment → Document source OR assumption
   ...
   **For sourced data, cite exactly where it came from:**
   - Example: "TWSE／FinMind Terminal - MSFT Equity DES, accessed 2024-10-02"
   - Example: "Q4 2024 年報 filing, page 42, line item 'Total Revenue'"
   - Example: "TWSE MCP（twstock） consensus estimate as of 2024-10-02"
   - **Include hyperlinks when possible**: Right-click cell → Link → paste URL to MOPS 公開資訊申報, data source, or report
...
❌ Missing hyperlinks to MOPS 公開資訊申報 or data sources when available
...
2. **Gather data** (60-90 minutes)
   - Pull from primary sources (TWSE MCP, TWSE MCP（twstock） MCP, FinMind MCP if available; otherwise TWSE／FinMind, SEC)
...
- **Link to source files**: Hyperlink to TWSE／FinMind screenshots or MOPS 公開資訊申報
...
- [ ] **Hyperlinks added to cells where applicable** (MOPS 公開資訊申報, data provider pages, reports)
...
- [ ] **Hyperlinks added where relevant** (公開資訊觀測站（MOPS） filings, TWSE／FinMind pages, research reports)
