---
name: dcf-model
description: 建立 DCF（現金流折現）估值模型：取得財報資料、建立現金流預估、計算 WACC、做敏感度分析，輸出專業 Excel 模型。觸發詞：DCF、現金流折現、內在價值、估值模型、合理股價。
---
...
1. **MCP Servers** (if configured) - Structured financial data from providers like FinMind
...
Shares outstanding,Source: 年報 FY2024 Page 45 Note 12
Historical revenue,Source: 年報 FY2024 Page 32 Consolidated Statements
...
2. **Gather historical financials**:
   - Check for available MCP servers (FinMind, etc.)
   - Request from user if not available via MCP
   - Manual extraction from 年報 if necessary
...
- **MCP servers**: If configured (FinMind for historical financials)
...
- **Manual extraction**: 公開資訊觀測站（MOPS） filings as fallback
