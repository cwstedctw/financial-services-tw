...
**When to Use:** Only reference this file when a model template specifically requires pulling data from MOPS 公開資訊申報 (年報, 季報). For templates that provide data directly or use other data sources, this reference is not needed.

---

## Extracting Data from SEC Filings (年報與季報)

When populating a model template with public company data, extract financials directly from MOPS 公開資訊申報.

### Step 1: Locate the Filing

1. Use 公開資訊觀測站（MOPS）: `https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=[TICKER]&type=年報`
2. For quarterly data, use `type=季報`
...
Within the 年報 or 季報, locate:
- **Item 8** (年報) or **Item 1** (季報): Financial Statements
...
- 年報 provides 3 years of IS/CF, 2 years of BS
- For 3rd year BS, pull from prior year's 年報
- Use 季報 to fill in quarterly granularity if needed
