...
description: 填寫與完成三表財務模型範本（綜合損益表、資產負債表、現金流量表），把資料填入並串接整合。當被要求填寫／完成／建立三表模型、串接整合財報時使用。
---
...
If the template specifically requires pulling data from MOPS 公開資訊申報 (年報, 季報), see [references/sec-filings.md](references/sec-filings.md) for detailed extraction guidance. This reference is only needed when populating templates with public company data from regulatory filings.
